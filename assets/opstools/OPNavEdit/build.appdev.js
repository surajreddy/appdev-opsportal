...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/OPNavEdit
        ...
        async.series([
            ...
            function (next) {
                ...
                AD.log('<green>building</green> opstools/OPNavEdit JS files');

                // Minify js/ejs files
                transform({
                    main: path.join('opstools', 'OPNavEdit', 'OPNavEdit'),
                    config: "stealconfig.js"
                }, {
                        minify: true,
                        ignore: [
                            /^.*(.css)+/, // Ignore css files
                            /^(?!opstools\/OPNavEdit.*)/, // Ignore all are not plugin scripts
                        ]
                    }).then(function (transform) {
                        // Get the main module and it's dependencies as a string
                        var main = transform();

                        fs.writeFile(path.join('opstools', 'OPNavEdit', 'production.js'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/OPNavEdit JS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/OPNavEdit JS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function (next) {
                AD.log('<green>building</green> opstools/OPNavEdit CSS files');
                ...
                transform({
                    ...
                    config: "stealconfig.js"
                        ...
                        fs.writeFile(path.join('opstools', 'OPNavEdit', 'production.css'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/OPNavEdit CSS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/OPNavEdit CSS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'OPNavEdit', 'production.js'), tag:'packages/OpsPortal-OPNavEdit.js', replace:'OpsPortal/production.js'}
                ];
