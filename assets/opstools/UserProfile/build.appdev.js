    ...
    command: function (builder, cb) {
        ...
        async.series([
            ...
            function (next) {
                ...
                // build command:  ./cjs steal/buildjs OpsPortal opstools/UserProfile

                AD.log('<green>building</green> opstools/UserProfile JS files');

                // Minify js/ejs files
                transform({
                    main: path.join('opstools', 'UserProfile', 'UserProfile'),
                    config: "stealconfig.js"
                        ...
                        ignore: [
                            ...
                            /^(?!opstools\/UserProfile.*)/, // Ignore all are not plugin scripts
                        ]
                        ...
                        fs.writeFile(path.join('opstools', 'UserProfile', 'production.js'), main.code, "utf8", function (err) {
                            if (err) {
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/UserProfile JS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function (next) {
                ...
                var patches = [
                    { file: path.join('opstools', 'UserProfile', 'production.js'), tag: 'packages/OpsPortal-UserProfile.js', replace: 'OpsPortal/production.js' }
                ];
